...
func NewProblemDetails(statusCode int, commonError *GeneralError, problemType, instance string) *ProblemDetails {
	if problemType == "" {
		problemType = "about:blank"
	}

	if problemType == "about:blank" && commonError.Message == "" {
		commonError.Message = http.StatusText(statusCode)
	}

	return &ProblemDetails{
		Type:          problemType,
		Title:         commonError.Message,
		Status:        statusCode,
		Detail:        commonError.SystemMessage,
		Instance:      instance,
		InvalidParams: commonError.ValidationErrors,
	}
}
